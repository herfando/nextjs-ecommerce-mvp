...
import { useAuth } from '@/lib/context/auth_context'; // Asumsi context Anda
...
const NavAuthSection = () => {
  // Asumsi useAuth sudah ada dan menyediakan 'user' (status login) dan 'logout'
  const { user, logout } = useAuth(); 
  const router = useRouter();

  if (user) {
    // KONDISI: User Sudah Login (sesuai Figma/Screenshot 2219b0.png)
    return (
      <div className="flex items-center gap-4">
        ...
        <Button variant="ghost" className="p-0 h-auto" onClick={() => console.log('Go to Profile')}>
            <Avatar className="w-8 h-8">
                <AvatarImage src="/images/placeholder-avatar.jpg" alt="Profile" />
  ...
  // KONDISI: User Belum Login (sesuai Figma/Screenshot 221955.png)
  return (
